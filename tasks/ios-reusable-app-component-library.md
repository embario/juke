...
Establish a reusable iOS app component library (JukeKit) that encapsulates common functionality across Juke, ShotClock, and TuneTrivia apps.

## Scope

- Create `JukeKit` Swift Package under `mobile/ios/Packages/JukeKit/`
...
- [x] Architecture document exists at `docs/arch/ios-jukekit-architecture.md`
- [x] JukeKit Swift Package created with proper structure
- [ ] All three apps consume JukeKit for shared functionality
...
- [x] Unit tests exist for JukeKit components (120 tests)
- [ ] iOS tests/builds pass for all three apps after refactor

## Execution Notes

- Key files:
  - Architecture doc: `docs/arch/ios-jukekit-architecture.md`
  - Package location: `mobile/ios/Packages/JukeKit/`
...
| 2026-02-11 | Design | Created architecture document at `docs/arch/ios-jukekit-architecture.md` |
| 2026-02-11 | Investigation | Audited SpotifyiOS SDK usage in ShotClock - can be replaced with backend playback |
| 2026-02-11 | Phase 1 | Created JukeKit Swift Package structure at `mobile/ios/Packages/JukeKit/` |
...
| 2026-02-11 | Phase 6 | Extracted Design System: JukeTheme protocol, built-in themes (JukeDefaultTheme, ShotClockTheme, TuneTriviaTheme), themeable components (JukeKitBackground, JukeKitCard, JukeKitButtonStyle, JukeKitInputField, JukeKitStatusBanner, JukeKitSpinner, JukeKitChip), Color(hex:) extension (92 tests passing) |
...
- Completed:
  ...
  - Phase 1: JukeKit Swift Package created at `mobile/ios/Packages/JukeKit/`
...
- Next:
  - Migrate Juke app to use JukeKit
  - Migrate ShotClock app to use JukeKit
  - Migrate TuneTrivia app to use JukeKit
